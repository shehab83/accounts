...
@section('content')
      ...
      <div class="card">
        <div class="card-header">
          <h3 class="card-title card_title_center">     فواتير مرتجع المشتريات العام  </h3>
          <input type="hidden" id="token_search" value="{{csrf_token() }}">
          <input type="hidden" id="ajax_search_url" value="{{ route('admin.suppliers_orders_general_return.ajax_search') }}">
          @can('اضافة فاتورة مرتجع مشتريات')
          <a href="{{ route('admin.suppliers_orders_general_return.create') }}" class="btn btn-sm btn-success" >اضافة جديد</a>
          @endcan
        </div>
        <!-- /.card-header -->
        <div class="card-body">
        ...
        <div id="ajax_responce_serarchDiv">
         ...
         <td>
          ...
          @can('تعديل فاتورة مترجع مشتريات')
        <a href="{{ route('admin.suppliers_orders_general_return.edit',$info->id) }}" class="btn btn-sm  btn-primary">تعديل</a>
        @endcan
        @can('حذف فاتورة مترجع مشتريات')
        <a href="{{ route('admin.suppliers_orders_general_return.delete',$info->id) }}" class="btn btn-sm are_you_shue  btn-danger">حذف</a>
        @endcan
        @endif
        @can('تفاصيل فاتورة مرتجع مشتريات')
        <a href="{{ route('admin.suppliers_orders_general_return.show',$info->id) }}" class="btn btn-sm   btn-info">التفاصيل</a>
        @endcan
        @can('طباعه فاتورة مرتجع مشتريات')
        <a style="font-size: .875rem; padding: 0.25rem 0.5rem;color:white" target="_blank" href="{{ route('admin.suppliers_orders_general_return.printsaleswina4',[$info->id,'A4']) }}" class="btn btn-primary btn-xs"> WA4</a>
        <a style="font-size: .875rem; padding: 0.25rem 0.5rem;color:white" target="_blank" href="{{ route('admin.suppliers_orders_general_return.printsaleswina4',[$info->id,'A6']) }}" class="btn btn-warning btn-xs"> WA6</a>
        @endcan
         </td>
