...
<a href="{{ route('admin.SalesReturnInvoices.index') }}">  مرتجع المبيعات العام  </a>
@endsection
...
<div class="card">
   <div class="card-header">
      <h3 class="card-title card_title_center">     فواتير مرتجع المبيعات العام للعملاء  </h3>
      <input type="hidden" id="token_search" value="{{csrf_token() }}">
      <input type="hidden" id="ajax_get_item_uoms" value="{{ route('admin.SalesReturnInvoices.get_item_uoms') }}">
      <input type="hidden" id="ajax_get_load_modal_addMirror" value="{{ route('admin.SalesReturnInvoices.load_modal_addMirror') }}">
      <input type="hidden" id="ajax_get_load_modal_addActiveInvoice" value="{{ route('admin.SalesReturnInvoices.load_modal_addActiveInvoice') }}">
      <input type="hidden" id="ajax_get_item_batches" value="{{ route('admin.SalesReturnInvoices.get_item_batches') }}">
      <input type="hidden" id="ajax_get_item_unit_price" value="{{ route('admin.SalesReturnInvoices.get_item_unit_price') }}">
      <input type="hidden" id="ajax_get_Add_new_item_row" value="{{ route('admin.SalesReturnInvoices.get_Add_new_item_row') }}">
      <input type="hidden" id="ajax_get_store" value="{{ route('admin.SalesReturnInvoices.store') }}">
      <input type="hidden" id="ajax_get_load_invoice_update_modal" value="{{ route('admin.SalesReturnInvoices.load_invoice_update_modal') }}">
      <input type="hidden" id="ajax_get_Add_item_to_invoice" value="{{ route('admin.SalesReturnInvoices.Add_item_to_invoice') }}">
      <input type="hidden" id="ajax_get_reload_items_in_invoice" value="{{ route('admin.SalesReturnInvoices.reload_items_in_invoice') }}">
      <input type="hidden" id="ajax_get_recalclate_parent_invoice" value="{{ route('admin.SalesReturnInvoices.recalclate_parent_invoice') }}">
      <input type="hidden" id="ajax_get_remove_active_row_item" value="{{ route('admin.SalesReturnInvoices.remove_active_row_item') }}">
      <input type="hidden" id="ajax_DoApproveInvoiceFinally" value="{{ route('admin.SalesReturnInvoices.DoApproveInvoiceFinally') }}">
      <input type="hidden" id="ajax_load_usershiftDiv" value="{{ route('admin.SalesReturnInvoices.load_usershiftDiv') }}">
      <input type="hidden" id="ajax_load_invoice_details_modal" value="{{ route('admin.SalesReturnInvoices.load_invoice_details_modal') }}">
      <input type="hidden" id="ajax_ajax_search" value="{{ route('admin.SalesReturnInvoices.ajax_search') }}">
      <input type="hidden" id="ajax_do_add_new_customer" value="{{ route('admin.SalesReturnInvoices.do_add_new_customer') }}">
      <input type="hidden" id="ajax_getlastaddedcustomer" value="{{ route('admin.SalesReturnInvoices.get_last_added_customer') }}">
      <input type="hidden" id="ajax_searchforcustomer" value="{{ route('admin.SalesReturnInvoices.searchforcustomer') }}">
      <input type="hidden" id="ajax_searchforitems" value="{{ route('admin.SalesReturnInvoices.searchforitems') }}">
      <button class="btn btn-sm btn-primary" id="LoadModalAddBtnActiveInvoice" >     اضافة فاتورة مرتجع</button>
   </div>
   <!-- /.card-header -->
   <div class="card-body">
      <div class="row">
                  ...
                  <tbody>
                     ...
                     <tr>
                        ...
                        <td>
                           ...
                           <a href="{{ route("admin.SalesReturnInvoices.delete",$info->id) }}" class="btn btn-sm are_you_shue  btn-danger">حذف</a>
                           @endif
                           <button data-autoserial="{{ $info->auto_serial }}"  class="btn btn-sm load_invoice_details_modal btn-info">عرض</button>
                           <a style="font-size: .875rem; padding: 0.25rem 0.5rem;color:white" target="_blank" href="{{ route('admin.SalesReturnInvoices.printsaleswina4',[$info->id,'A4']) }}" class="btn btn-primary btn-xs"> WA4</a>
                           <a style="font-size: .875rem; padding: 0.25rem 0.5rem;color:white" target="_blank" href="{{ route('admin.SalesReturnInvoices.printsaleswina4',[$info->id,'A6']) }}" class="btn btn-warning btn-xs"> WA6</a>
   ...
   <div class="modal fade  "   id="AddNewInvoiceModalActiveInvoice">
      <div class="modal-dialog modal-xl" >
         <div class="modal-content bg-info">
            <div class="modal-header">
               <h4 class="modal-title text-center">          اضافة فاتورة مرتجع مبيعات عام  </h4>
               <button type="button" class="close" data-dismiss="modal" aria-label="Close">
               <span aria-hidden="true">&times;</span></button>
            </div>
