...
if(!empty($AccountData)){
 ...
 return view('admin.collect_transactions.get_account_blance',['the_final_Balance'=>$the_final_Balance]);
    ...
    $the_final_Balance =  refresh_account_blance_delegate($account_number,new Account(),new Delegate(),new Treasuries_transactions(),new services_with_orders(),new Sales_invoices(),true);
    return view('admin.collect_transactions.get_account_blance',['the_final_Balance'=>$the_final_Balance]);
...
public function ajax_search(Request $request)
{
    if ($request->ajax()) {
        ...
        $totalCollectInSearch = Treasuries_transactions::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where($field3, $operator3, $value3)->where($field4, $operator4, $value4)->where($field5, $operator5, $value5)->where($field6, $operator6, $value6)
