    ...
    public function delegate_account_mirror(Request $request)
    {
        if ($_POST) {
            ...
            if ($request->report_type == 1) {
                ...
                $delegateData['Does_show_items'] = $request->Does_show_items;
                $delegateData['the_final_Balance'] = refresh_account_blance_delegate($delegateData['account_number'], new Account(), new Delegate(), new Treasuries_transactions(), new Sales_invoices(), new services_with_orders(), true);
                $delegateData['ServicesForUsCounter'] = services_with_orders::where(["com_code" => $com_code, 'is_account_number' => 1, 'account_number' => $delegateData['account_number'], 'order_type' => 1])->where('order_date', '>=', $delegateData['from_date'])->where('order_date', '<=', $delegateData['to_date'])->count();
                $delegateData['ServicesForotherCounter'] = services_with_orders::where(["com_code" => $com_code, 'is_account_number' => 1, 'account_number' => $delegateData['account_number'], 'order_type' => 2])->where('order_date', '>=', $delegateData['from_date'])->where('order_date', '<=', $delegateData['to_date'])->count();
                $delegateData['ServicesForUsMoney'] = services_with_orders::where(["com_code" => $com_code, 'is_account_number' => 1, 'account_number' => $delegateData['account_number'], 'order_type' => 1])->where('order_date', '>=', $delegateData['from_date'])->where('order_date', '<=', $delegateData['to_date'])->sum('money_for_account');
                $delegateData['ServicesForothermoney'] = services_with_orders::where(["com_code" => $com_code, 'is_account_number' => 1, 'account_number' => $delegateData['account_number'], 'order_type' => 2])->where('order_date', '>=', $delegateData['from_date'])->where('order_date', '<=', $delegateData['to_date'])->sum('money_for_account');
                $delegateData['ServicesForUsMoney'] = $delegateData['ServicesForUsMoney'] * (-1);
                ...
                $systemData = get_cols_where_row(new Admin_panel_setting(), array("system_name", "phone", "address", "photo"), array("com_code" => $com_code));
                $delegateData['report_type'] = $request->report_type;
                return view('admin.financialReport.delegate.print_delegate_account_Services', ['data' => $delegateData, 'systemData' => $systemData, 'details' => $details]);
            } else {
                $delegateData['from_date'] = $request->from_date;
                $delegateData['to_date'] = $request->to_date;
                $delegateData['the_final_Balance'] = refresh_account_blance_delegate($delegateData['account_number'], new Account(), new Delegate(), new Treasuries_transactions(), new Sales_invoices(), new services_with_orders(), true);
