...
class Services_with_ordersController extends Controller
{
  ...
  //اعتماد وترحيل فاتورة الخدمات
  function do_approve($id, Request $request)
  {
      $com_code = auth()->user()->com_code;
      //check is not approved
      $data = get_cols_where_row(new services_with_orders(), array("total_services", "is_approved", "id","is_account_number", "account_number","entity_name", "auto_serial","order_type"), array("id" => $id, "com_code" => $com_code ));
      if (empty($data)) {
          return redirect()->route("admin.Services_orders.index")->with(['error' => "عفوا غير قادر علي الوصول الي البيانات المطلوبة !!"]);
      }

      if ($data['is_approved'] == 1) {
          return redirect()->route("admin.Services_orders.store", $data['id'])->with(['error' => "عفوا لايمكن اعتماد فاتورة معتمده من قبل !!"]);
      }
      $counterDetails = get_count_where(new services_with_orders_details(), array('services_with_orders_auto_serial' => $data['auto_serial'], 'order_type' => $data['order_type'], 'com_code' => $com_code));
      if ($counterDetails== 0) {
          return redirect()->route("admin.Services_orders.show", $data['id'])->with(['error' => "عفوا لايمكن اعتماد الفاتورة قبل اضافة الخدمات عليها !!!            "]);
      }
        ...
        if ($data['is_account_number'] == 1) {
          ...
          if ($request['what_paid'] > 0) {
              ...
              if ($data['order_type']==1){
                ...
                $dataInsert_treasuries_transactions['byan'] = " صرف نظير فاتورة خدمات مقدم لنا  رقم الفاتوره"  . $data['auto_serial'];
                }else{
                ...
                $dataInsert_treasuries_transactions['byan'] = " تحصيل نظير فاتورة خدمات مقدمها للغير  رقم الفاتوره" .$data['auto_serial'];
                }else{
