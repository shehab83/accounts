...
class SalesInvoicesController extends Controller
{
    ...
    public function searchforitems(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($searchtext != "") {
                ...
                $item_cards = Inv_itemCard::where('name', 'like', "%{$searchtext}%")->orWhere('product_code', 'like', "%{$searchtext}%")->orWhere('item_code', '=', $searchtext)->orderby('id', 'asc')->limit(10)->get();
    ...
    // public function printsaleswina6($id)
    // {
    //     $com_code = auth()->user()->com_code;
    //     $invoice_data = get_cols_where_row(new Sales_invoices(), array("*"), array("com_code" => $com_code, "id" => $id));
    //     if (empty($invoice_data)) {
